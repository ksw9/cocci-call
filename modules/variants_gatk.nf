process VariantsGATK {
  ...
  //tuple val(sample_id), val(batch), path("${sample_id}_gatk_unfilt.vcf.gz"), emit: gatk_vcf_unfilt
  tuple val(sample_id), val(batch), path("${sample_id}_gatk_filt.vcf.gz"), emit: gatk_vcf_filt
